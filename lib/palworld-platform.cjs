 ...
 *
 * The host is the machine Fleetdeck runs on. The target is the platform the
 ...
 * from the host alone, and Fleetdeck never installs Wine on the operator's
...
function safeWine(value = {}) {
  ...
  const env = {};
  const entries = Object.entries(input.env && typeof input.env === 'object' ? input.env : {}).slice(0, MAX_WINE_ENV);
 ...
 * have to go through; `supported` is whether Fleetdeck is willing to run it at
...
function compatibility({ server, wine, host = hostPlatform(), wineDetection = null } = {}) {
  ...
  if (target === 'windows' && host === 'linux') {
    ...
    if (!detection.available) {
      return { ...base, runtime: RUNTIMES.WINE, supported: false, reason: 'wine_not_detected', explanation: 'The configured Wine executable could not be run. Install or correct it; Fleetdeck does not install Wine.' };
    }
