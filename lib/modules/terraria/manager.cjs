...
// variant Fleetdeck knows. Callers that gate behavior fail closed on null; they
...
/*
 * The one command Fleetdeck types at a Terraria console on a timer.
 *
...
function createTerrariaModule(deps = {}) {
  ...
  function installerOptions(progress = () => {}) {
    return {
      ...(installerCacheDir ? { cacheDir: installerCacheDir } : {}),
  ...
  return {
    ...
    start(manager) {
      ...
      const args = Array.isArray(desc.args) ? [...desc.args] : [];
      // A .NET apphost ignores PATH, so a runtime Fleetdeck could only find
    ...
    async preLaunch(manager) {
      ...
      catch (_) { return { ok: true }; } // A failed probe must never block a legitimate start.
    ...
    inspectLine(line, manager) {
      ...
      if (consoleGrammar.isWorldSelectionPrompt(variant, line)) {
        if (!st.awaitingWorldSelection) {
          ...
          manager.pushLine('[Fleetdeck] This server is waiting for a world to be selected on its console and will not finish starting until one is. Fleetdeck will not answer the prompt for you: the menu is numbered by the world list, so a blind answer can load the wrong world. Choose a world from the worlds view.', 'warn');
      ...
      switch (event.kind) {
        ...
        case 'worldLoadFailed':
          manager.pushLine('[Fleetdeck] The server could not open its world file. Check the world configured for this server.', 'error');
          break;
    ...
    onExit(manager) {
      ...
      if (!clean && manager.manualStop) {
        manager.pushLine('[Fleetdeck] The server exited without confirming that it saved and shut down cleanly.', 'warn');
      }
      clearState(st);
    },
