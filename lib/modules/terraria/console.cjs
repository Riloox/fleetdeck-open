...
const RULES = Object.freeze([
  ...
  // -- players --------------------------------------------------------------
  {
    id: 'join',
    variants: ['vanilla', 'tshock', 'tmodloader'],
    // Vanilla and tModLoader: "Fleetdeck Guest has joined."
    // TShock also logs "Fleetdeck Guest has joined. IP: 127.0.0.1" and the
    // group form "Fleetdeck Guest (N/A) has joined."; both are handled by
    ...
    // unmodded client" - tmodloader-start.log), and Fleetdeck has no modded
  ...
  },
  {
    ...
    // "Fleetdeck Guest (127.0.0.1:44768)" - one line per player, address last.
  ...
  },
  {
    id: 'player-limit',
    variants: ['vanilla', 'tshock', 'tmodloader'],
    // The reply to the `maxplayers` console command. Fleetdeck does not poll
 ...
 * parentheses ("Fleetdeck Guest (N/A) has joined.") and once as a log line
 * with the address ("Fleetdeck Guest has joined. IP: 127.0.0.1"). Both name
