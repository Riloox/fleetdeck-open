 ...
 * a server folder Fleetdeck installed and a folder an operator brings from
 * their own machine resolve differently. Phase 3 reads it; there is deliberately
 * no Fleetdeck-side default directory to fall back to.
 */
...
const ENTRIES = Object.freeze({
  ...
  tmodloader: Object.freeze({
    id: 'tmodloader',
    label: 'tModLoader',
    // tModLoader ships .sh/.bat launcher wrappers that Fleetdeck must never
 ...
 * server Fleetdeck could register back then was vanilla - so that is a
