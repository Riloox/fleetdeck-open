...
function normalizeStatus(manager) {
  ...
  let range = null;
  try { range = launch.portPlan(desc).range; } catch (_) {}
  ...
  return {
    ...
    degradedReason: st.degradedReason || (range ? 'Valheim port-span evidence is not settled; Fleetdeck reserves three ports conservatively.' : null),
  };
}
...
function createValheimModule(deps = {}) {
  ...
  return {
    ...
    async preLaunch(manager) {
      ...
      for (const other of getConfig().servers || []) {
        ...
        if (other.type === 'valheim') {
          try { otherPorts = launch.portPlan(other).ports; } catch (_) { continue; }
        } else {
      ...
      for (const port of plan.ports) {
        let used = false;
        try { used = await probePortInUse(port, '0.0.0.0'); } catch (_) { continue; }
    ...
    onReadinessTimeout(manager) {
      ...
      manager.pushLine('[Fleetdeck] Valheim startup timed out. The process is still running; inspect its logs or stop it explicitly.', 'warn');
    },
