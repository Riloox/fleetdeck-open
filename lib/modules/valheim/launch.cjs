...
function realInside(root, candidate, label, allowMissing = false) {
  ...
  try { realRoot = fs.realpathSync.native(root); } catch (_) {}
  try {
...
function migrateDescriptor(input) {
  ...
  return {
    ...
    serverName: parsed.values.serverName ?? input.serverName ?? 'Fleetdeck server',
...
function buildLaunch(input, host = process.platform) {
  ...
  for (const arg of extra) {
    ...
    if (OWNED_FLAGS.has(text.toLowerCase())) throw new Error(`Fleetdeck-owned option cannot appear in extra arguments: ${text}`);
  }
  const args = [
    '-name', serverName,
