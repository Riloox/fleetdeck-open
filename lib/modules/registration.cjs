...
function executableInDirectory(dir, value) {
  ...
  if (process.platform !== 'win32') {
    ...
    catch (_) { throw new Error('Executable is not executable'); }
  }
  return executable;
}
 ...
 *
 * Omitting it means vanilla - the only Terraria server Fleetdeck could install
...
function validateManualRegistration(body, { maxNameLength = 80 } = {}) {
  ...
  if (healthCheckRegex) new RegExp(healthCheckRegex);
