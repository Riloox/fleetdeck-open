...
function createPalworldModule(deps = {}) {
  ...
  const panelName = () => (typeof deps.getConfig === 'function'
    ? (deps.getConfig().appName || 'Fleetdeck')
    : 'Fleetdeck');
  ...
  return {
    ...
    start(manager) {
      ...
      // which Fleetdeck spawns directly (never through a shell).
    ...
    buildStopSequence(manager) {
      return {
        execute: async () => {
          ...
          manager.pushLine('[Fleetdeck] Palworld accepted the shutdown request.', 'info');
        },
      };
    },
    ...
    async mutate(manager, action, body) {
      ...
      return { ok: true, result: result && typeof result === 'object' ? { accepted: true } : null };
    },
