...
function createMinecraftModule(deps) {
  ...
  return {
    ...
    start(manager) {
      ...
      manager.pushLine(`[Fleetdeck] Minecraft ${d.mcVersion || '?'} needs Java ${major}. Downloading runtime (one-time)...`, 'info');
      let lastPct = -1;
      ensureRuntime(major, (rec, total) => {
        if (!total) return;
        const pct = Math.floor((rec / total) * 100);
        if (pct >= lastPct + 10) { lastPct = pct; manager.pushLine(`[Fleetdeck] Downloading Java ${major}: ${pct}%`, 'info'); }
      }).then((bin) => {
        manager._runtimeFetching = false;
        manager.pushLine(`[Fleetdeck] Java ${major} runtime ready.`, 'info');
        const r = manager._launch(bin, args);
        if (!r.ok) { manager.setStatus('offline'); manager.pushLine(`[Fleetdeck] Could not launch java: ${r.error}`, 'error'); }
      }).catch((err) => {
        manager._runtimeFetching = false;
        manager.setStatus('offline');
        manager.pushLine(`[Fleetdeck] Could not prepare Java ${major}: ${err.message}`, 'error');
      });
      return { ok: true };
    },
