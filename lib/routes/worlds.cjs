...
module.exports = function worldsRouter(deps) {
  ...
  // Imports and world mutations are expensive and destructive, so each actor
  // gets a per-user budget (canonical helper: lib/palworld-operations.cjs).
  // Authorization is untouched: capability checks still run first, and the
  // idempotency replay path always answers before the limiter is consulted.
  const limitImports = palworldOperations.createRateLimiter({ limit: 10, windowMs: 60_000 });
  const limitWorldMutations = palworldOperations.createRateLimiter({ limit: 10, windowMs: 60_000 });
  ...
  router.post('/import/preview', requireCap(CAPABILITIES.WORLDS_MANAGE, scope), (req, res) => {
    // Gate before multer reads the body: a 16 GiB upload must not be accepted
    // from an actor who is already over budget.
    ...
    upload(req, res, async (uploadError) => {
      ...
      try {
        ...
        // generated; assert its shape so the rename target is pinned inside
        ...
        fs.renameSync(staged, worlds.importStagingDir(token) + '.zip');
  ...
  router.post('/:id/clone', requireCap(CAPABILITIES.WORLDS_MANAGE, scope), (req, res) => {
    ...
    if (!world) return;
    const rate = limitWorldMutations(req.user.id);
    if (!rate.allowed) return tooMany(res, rate);
    try {
