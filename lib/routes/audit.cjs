...
module.exports = function auditRouter() {
  ...
  // often one account may run them (CodeQL js/missing-rate-limiting).
  const limitRetention = palworldOperations.createRateLimiter({ limit: 10, windowMs: 60_000 });
  ...
  router.post('/retention/preview', requireCap(CAPABILITIES.AUDIT_RETENTION), (req, res) => {
    const rate = limitRetention(`preview:${req.user.id}`);
  ...
  router.post('/retention/apply', requireCap(CAPABILITIES.AUDIT_RETENTION), (req, res) => {
    const rate = limitRetention(`apply:${req.user.id}`);
    if (!rate.allowed) return rateLimited(res, rate);
