 ...
 * X-Fleetdeck-Server-Id headers and config.activeServerId are ignored here.
 *
...
module.exports = function bugReportsRouter(deps) {
  ...
  // this only stops multi-hundred-KB junk before it is processed.
  ...
  router.post('/bug-reports', async (req, res) => {
    ...
    recordAudit({
      ...
      metadata: {
        reportId: report.id,
        state: sync.state,
        ...(sync.issueNumber != null ? { issueNumber: sync.issueNumber } : {}),
        ...(sync.issueUrl != null ? { issueUrl: sync.issueUrl } : {}),
        ...(sync.error != null ? { error: sync.error } : {}),
      },
    });
