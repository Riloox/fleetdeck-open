...
const IMPORT_DIR = path.join(os.tmpdir(), 'fleetdeck-template-imports');
const upload = multer({
  dest: IMPORT_DIR,
...
function tooMany(res, rate) {
  res.setHeader('Retry-After', String(Math.ceil(rate.retryAfterMs / 1000)));
  return res.status(429).json({ error: 'Too many template operations. Try again shortly.', code: 'rate_limited' });
}

/*
 * multer names the uploaded file itself (random hex) inside a server-owned
 * directory, so the client never controls the on-disk path. Re-assert that
 * shape before any fs call so a request-derived value can never reach a path
 * sink unguarded (CodeQL js/path-injection).
 */
function stagedUploadPath(file) {
  const abs = path.resolve(file && file.path ? file.path : '');
  const root = path.resolve(IMPORT_DIR);
  if (path.dirname(abs) !== root || !/^[0-9a-f]{32}$/i.test(path.basename(abs))) {
    throw Object.assign(new Error('Upload staging is misconfigured.'), { status: 500, code: 'staging_misconfigured' });
  }
  return abs;
}
...
function build(deps) {
  ...
  async function run({ operationId, load, parentDir, name, placeholders, sourceServerId, templateId }) {
    // slugFor can only produce [a-z0-9-], so the destination is pinned under
    // the validated parentDir; assert the invariant explicitly before any fs
    // call so a request-derived path can never escape it (CodeQL
    // js/path-injection).
    const slug = templates.slugFor(name);
    const destination = path.join(parentDir, slug);
    if (path.basename(destination) !== slug || path.dirname(destination) !== parentDir) {
      throw Object.assign(new Error('Invalid destination path.'), { status: 400, code: 'invalid_path' });
    }
  ...
  function dispatch(req, res, { kind, action, sourceServerId, templateId, load, summary }) {
    ...
    const rawParent = String(req.body?.parentDir || '');
    // The destination folder is the admin's explicit choice, but it must be an
    // absolute, traversal-free path: path.join(parentDir, slug) may never
    // escape it. Relative paths are rejected rather than resolved against the
    // panel's own working directory.
    if (!rawParent || !path.isAbsolute(rawParent) || rawParent.split(/[\\/]+/).includes('..')
      || !fs.existsSync(rawParent) || !fs.statSync(rawParent).isDirectory()) {
      return res.status(400).json({ error: 'Destination folder was not found.', code: 'destination_missing' });
    }
    const parentDir = path.resolve(rawParent);
...
function router(deps) {
  ...
  // Template operations are expensive (archives, full server roots) or
  // destructive, so each actor gets a per-user budget per operation
  // (canonical helper: lib/palworld-operations.cjs). Authorization is
  // untouched: the capability checks below still run first.
  const limitCurate = palworldOperations.createRateLimiter({ limit: 10, windowMs: 60_000 });
  const limitImports = palworldOperations.createRateLimiter({ limit: 10, windowMs: 60_000 });
  const limitInstantiate = palworldOperations.createRateLimiter({ limit: 10, windowMs: 60_000 });
  const limitRecover = palworldOperations.createRateLimiter({ limit: 10, windowMs: 60_000 });
  const limitDelete = palworldOperations.createRateLimiter({ limit: 10, windowMs: 60_000 });
  ...
  r.post('/', async (req, res) => {
    try {
      ...
      if (!server) return res.status(404).json({ error: 'Server not found.' });
      if (!canCurate(req.user, server.id)) return deny(res, CAPABILITIES.CONTENT_VIEW);
      const rate = limitCurate(req.user.id);
      if (!rate.allowed) return tooMany(res, rate);
  ...
  r.post('/import/preview', upload.single('file'), async (req, res) => {
    const rate = limitImports(req.user.id);
    if (!rate.allowed) return tooMany(res, rate);
    // multer names the file itself inside a server-owned directory; only the
    // validated staging path may reach the filesystem (CodeQL js/path-injection).
    const staged = req.file ? stagedUploadPath(req.file) : null;
    const discard = () => { if (staged) fs.rmSync(staged, { force: true }); };
    if (req.user?.role !== 'admin') { discard(); return deny(res, CAPABILITIES.SERVER_MANAGE); }
    try {
      res.json(await templates.importPreview(staged, req.user.id));
  ...
  r.post('/:id/instantiate', (req, res) => {
    ...
    const rate = limitInstantiate(req.user.id);
    if (!rate.allowed) return tooMany(res, rate);
    dispatch(req, res, {
  ...
  r.post('/operations/:id/recover', (req, res) => {
    ...
    const rate = limitRecover(req.user.id);
    if (!rate.allowed) return tooMany(res, rate);
  ...
  r.delete('/:id', (req, res) => {
    try {
      ...
      if (!row) return res.status(404).json({ error: 'Template not found.' });
      if (!canCurate(req.user, row.source_server_id)) return deny(res, CAPABILITIES.CONTENT_VIEW);
      const rate = limitDelete(req.user.id);
      if (!rate.allowed) return tooMany(res, rate);
...
function cloneRouter(deps) {
  ...
  const limitClone = palworldOperations.createRateLimiter({ limit: 10, windowMs: 60_000 });
  ...
  r.post('/:id/clone', (req, res) => {
    ...
    if (!source) return res.status(404).json({ error: 'Server not found.' });
    if (!canManage(req.user, source.id)) return deny(res, CAPABILITIES.SERVER_MANAGE);
    const rate = limitClone(req.user.id);
    if (!rate.allowed) return tooMany(res, rate);
    dispatch(req, res, {
      kind: 'template-clone',
