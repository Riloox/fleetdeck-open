...
/*
 * Managed content for a server: the provenance rows written when Fleetdeck
...
async function create({ server, name, description, actorId, templateId = null }) {
  ...
  const archive = path.join(TEMPLATE_DIR, ...relArchive.split('/'));
        ...
        zip.openReadStream(entry, (streamErr, stream) => {
          ...
          stream.on('end', () => {
            ...
            if (normalized === 'manifest.json') {
              ...
              catch (_) { return fail(error('Template manifest is invalid.', 400, 'invalid_manifest')); }
            } else {
...
/*
 * An imported archive is untrusted even if Fleetdeck exported it: the manifest
...
function validateImported(manifest, files) {
  if (!manifest || manifest.schemaVersion !== SCHEMA_VERSION) {
    ...
    throw error(
      newer ? 'Template schema is newer than this Fleetdeck version.' : 'Unsupported template schema.',
      400,
      'unsupported_schema',
    );
  }
...
async function modrinthVersion(versionId) {
  const response = await fetch(`${MODRINTH}/version/${encodeURIComponent(versionId)}`, {
    headers: { 'user-agent': 'Fleetdeck/1.0' },
  });
