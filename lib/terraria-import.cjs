...
function inspect(dir, { servers = [], variant = null } = {}) {
  ...
  const issues = [];
  if (detected.length > 1 && !selected) issues.push({ code: 'variant_ambiguous', severity: 'error', detail: `Markers for ${choices.join(' and ')} are both present. Choose which runtime Fleetdeck should launch.` });
