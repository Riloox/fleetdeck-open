 ...
 * Fleetdeck downloads contain the game, not the runtime that runs it. When the
 ...
 * a refusal Fleetdeck can make before spawning, from evidence rather than
 * remembered trivia:
 *
...
function frameworkFromConfig(text) {
  ...
  const options = parsed && typeof parsed === 'object' ? parsed.runtimeOptions : null;
...
function rootFromPath(findRuntime) {
  ...
  catch (_) { return path.dirname(String(found)); }
}
 ...
 * point - Fleetdeck must refuse exactly what the apphost would refuse, no more.
 */
