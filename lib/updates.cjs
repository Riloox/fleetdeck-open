...
async function apiJson(url, options = {}) {
  const r = await fetch(url, { ...options, headers: { 'user-agent': 'Fleetdeck/1.0', 'content-type': 'application/json', ...(options.headers || {}) } });
...
function router({ findServer, getManager, detectCompat }) {
  ...
  // hammer the panel or Modrinth (CodeQL js/missing-rate-limiting).
  const limitUpdateActions = palworldOperations.createRateLimiter({ limit: 20, windowMs: 60_000 });
  const limitPlanReads = palworldOperations.createRateLimiter({ limit: 60, windowMs: 60_000 });
  function rateLimited(res, rate) {
  ...
  r.post('/scan', requireCap(CAPABILITIES.UPDATES_VIEW, { getServerId: sid }), async (req, res) => {
    ...
    const rate = limitUpdateActions(`${req.user.id}:${s.id}`);
    if (!rate.allowed) return rateLimited(res, rate);
  ...
  r.get('/plans/:id', (req, res) => {
    const rate = limitPlanReads(`plan:${req.user.id}`);
    if (!rate.allowed) return rateLimited(res, rate);
  ...
  r.post('/plans/:id/apply', async (req, res) => {
    const rate = limitUpdateActions(`apply:${req.user.id}`);
    if (!rate.allowed) return rateLimited(res, rate);
  ...
  r.post('/plans/:id/rollback', async (req, res) => {
    const rate = limitUpdateActions(`rollback:${req.user.id}`);
    if (!rate.allowed) return rateLimited(res, rate);
    const db = open(); const row = db.prepare('SELECT * FROM update_plans WHERE id=?').get(req.params.id);
    if (!row || !require('./capabilities.cjs').has(req.user, row.server_id, CAPABILITIES.UPDATES_APPLY)) return res.status(404).json({ error: 'Plan not found' });
    const s = findServer(row.server_id); const m = s && getManager(s.id); const plan = json(row.plan_json, {});
