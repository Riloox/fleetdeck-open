...
const MIGRATIONS = [
  ...
  },
  {
    ...
    up(db) {
      db.exec(`
        -- In-app bug reports. The row is durable BEFORE any GitHub call, so a
        -- failed sync leaves a retryable record instead of losing the report;
        -- lib/bug-report-sync.cjs owns the retry loop and writes back here.
        ...
        CREATE INDEX IF NOT EXISTS bug_reports_sync_state_idx ON bug_reports(sync_state);
        CREATE INDEX IF NOT EXISTS bug_reports_created_at_idx ON bug_reports(created_at);
        -- Idempotency marker: unique so a double-submit resolves to the existing
        -- row. NULLs stay distinct in SQLite, so a marker-less row is unaffected.
        CREATE UNIQUE INDEX IF NOT EXISTS bug_reports_marker_unique ON bug_reports(marker);
      `);
    },
  },
];
