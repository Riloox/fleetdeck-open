 ...
 *     never a Fleetdeck-side guess about where worlds live.
...
/*
 * A world name is one path segment Fleetdeck is willing to create on any of the
 ...
 * would be Fleetdeck inventing a restriction the filesystem does not have.
 ...
 * then a `-worldpath` launch flag. There is deliberately no Fleetdeck-side
 * default (see lib/modules/terraria/variants.cjs): a folder Fleetdeck installed
...
function resolveSaveDir(desc) {
  ...
  fail(
    'Fleetdeck cannot tell where this server keeps its worlds. Set its world folder (the "worldpath" setting) and try again.',
    409,
    'save_dir_unknown',
  );
  return null;
...
function worldPath(saveDir, file) {
  ...
  catch (_) { fail('That world path is not inside the world folder.', 400, 'path_escape'); }
 ...
 * agree: the descriptor Fleetdeck owns and the `world=` line the server reads.
...
function currentSelection(desc, saveDir) {
  ...
  return {
    ...
    descriptor: stored,
    config: configured,
    // Only a contradiction is a disagreement. A server created by Fleetdeck has
 ...
 *
 * The check is against the whole save directory, not just the worlds Fleetdeck
...
/*
 * Uploads are staged under the Fleetdeck data directory, never inside a server
 ...
 * upload into a directory under the Fleetdeck data dir, never into the server
...
function importRejection(reason) {
  switch (reason) {
    ...
    case 'unsupported_version': return 'That world header carries a format version Fleetdeck does not recognise, so it will not be imported.';
...
function previewGenerate({ desc, actorId, input, manager }) {
  ...
  // Generation runs the server binary. A variant Fleetdeck cannot identify has
  // no known launch plan, and guessing one is how the wrong binary gets run.
  const variant = safeVariant(desc);
  if (!variant) fail('This server\'s Terraria edition is not recognised, so Fleetdeck will not run its binary.', 409, 'unknown_variant');
...
function generationLaunch(desc, configFile) {
  ...
  if (!executable) fail('This server has no executable configured, so Fleetdeck cannot generate a world for it.', 409, 'executable_missing');
  if (!fs.existsSync(executable)) fail('This server\'s executable is missing, so Fleetdeck cannot generate a world for it.', 409, 'executable_missing');
  if (WRAPPER_EXTENSIONS.has(path.extname(executable).toLowerCase())) {
    fail('Fleetdeck refuses to start a Terraria server through a launcher script.', 409, 'wrapper_refused');
  }
