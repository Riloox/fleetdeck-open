 ...
 * Branding resolution.
 *
 * A hosting provider shipping Fleetdeck under their own name has to be able to
...
function resolveGameAccents(config) {
  const raw = (config && typeof config.gameAccents === 'object' && config.gameAccents) || {};
  const out = {};
  for (const id of GAME_IDS) out[id] = deriveGameTheme(raw[id]) ?? null;
  return out;
}

// The hex shape accepted for a game accent, matching lib/serverPresentation.cjs
...
function resolve(config) {
  const branding = (config && typeof config.branding === 'object' && config.branding) || {};
