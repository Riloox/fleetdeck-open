...
/*
 * GitHub Issues client (plan Task 3 — .hermes/plans/2026-08-14_235510-report-bug-github.md).
 *
 * Native fetch only, with injectable fetch/baseUrl for tests — no live
 * network in the test suite. Credentials are server-side only: the token
 ...
 * The rest of the text passes through untouched and readable.
 */
...
function buildIssueBody(input = {}) {
  const parts = [
    ...
    '',
    '## Reported by',
    escapeMarkdown(input.actorUsername ? `${input.actorUsername} (${input.actorId})` : valueOrDash(input.actorId)),
    '',
    '## Timestamp',
...
function createGitHubClient(deps = {}) {
  ...
  if (!token) throw new Error('github-issues: token is required');
  if (!owner) throw new Error('github-issues: owner is required');
  if (!repo) throw new Error('github-issues: repo is required');
  ...
  const userAgent = `Fleetdeck/${deps.version || '1.0'} (bug-report sync)`;
