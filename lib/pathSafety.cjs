 ...
 *
 * Spec contract:
 *   - "Reject drive roots, home/profile roots, Fleetdeck data/build
...
/*
 * Directories Fleetdeck refuses to treat as a server root, whether adopting or
 * removing. Each entry carries the reason the caller shows the operator: "not
 * allowed" without a why is an unactionable error.
 *
 * `strict` entries also protect everything below them - Fleetdeck's own data,
...
const REASONS = Object.freeze({
  ...
  fleetdeck_data: 'This is the Fleetdeck data folder.',
  fleetdeck_install: 'This is the Fleetdeck installation folder.',
  backup_root: 'This is the Fleetdeck backup folder.',
  protected: 'This folder is protected.',
  contains_protected: 'This folder contains a protected Fleetdeck folder.',
...
function protectedReason(target, { servers = [], selfId = null, extra = [], requireExisting = false } = {}) {
  ...
  // than "this contains some Fleetdeck folder", and a home folder containing
  // the Fleetdeck install would otherwise report the vaguer reason.
