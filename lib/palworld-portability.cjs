 ...
 *
 * Spec contract:
 *   - "Define a versioned Fleetdeck Palworld profile archive containing:
 ...
 * Fleetdeck never regenerates the file from a template.
 */
...
function redactedMetadata(server, extras = {}) {
  return {
    ...
    watchdog: server.watchdog && typeof server.watchdog === 'object'
      ? {
        ...
        zip.openReadStream(entry, (streamError, stream) => {
          ...
          out.on('close', () => {
            ...
            if (rel === 'manifest.json') {
              ...
              catch (_) { return abort(new ProfileError('The profile manifest is not valid JSON.', 400, 'invalid_manifest')); }
            }
            zip.readEntry();
          });
        });
...
function validateManifest(manifest, written) {
  if (!manifest || manifest.format !== FORMAT) fail('That archive is not a Fleetdeck Palworld profile.', 400, 'unsupported_format');
  ...
  if (version > PROFILE_VERSION) fail('This profile was written by a newer Fleetdeck version.', 400, 'unsupported_version');
...
async function importPreview({ file, actorId = null, servers = [], now = Date.now() }) {
  ...
  return {
    ...
    nextSteps: [
      ...
      'Fleetdeck generates a new administration password; the profile never carried one.',
...
/*
 * Everything Fleetdeck can learn about an existing installation without
 * changing a single byte of it. `reconcile` is the honest part: the REST
 * settings Fleetdeck needs are listed *before* adoption so the operator can see
...
function inspectAdoption({ dir, servers = [], desiredRestPort = null }) {
  ...
  const reconcile = [];
  if (!restEnabled) {
    reconcile.push({ key: 'RESTAPIEnabled', current: false, next: true, why: 'Fleetdeck manages Palworld through the loopback REST API.' });
  }
  if (!restPort || (wantedRestPort && restPort !== wantedRestPort)) {
    reconcile.push({ key: 'RESTAPIPort', current: restPort, next: wantedRestPort, why: 'Fleetdeck needs a known REST port to reach this server.' });
  }
  if (!adminPassword) {
    reconcile.push({ key: 'AdminPassword', current: null, next: 'generated', why: 'No administration password is set. Fleetdeck generates one and keeps it out of the browser.' });
  } else {
