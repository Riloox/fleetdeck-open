...
/*
 * Safe configuration for the GitHub-issue bug-report sync
 * (plan Task 1 — .hermes/plans/2026-08-14_235510-report-bug-github.md) and
 * the upstream-relay mode (plan Task 5 — .hermes/plans/2026-08-15_124205-upstream-bug-report-relay-pi.md).
 *
...
function normalizeToken(input, env) {
  const envValue = env && typeof env === 'object' ? env[GITHUB_TOKEN_ENV] : undefined;
...
async function syncReportToRelay(report, deps = {}) {
  ...
  const code = typeof body.error === 'string'
    ? body.error
    : (body.error && typeof body.error === 'object' && typeof body.error.code === 'string' ? body.error.code : null);
