 ...
 *   2. **No shell.** tModLoader and TShock ship `.sh`/`.bat` wrappers. Fleetdeck
...
const USER_AGENT = 'Fleetdeck/1.0';
...
// Launcher wrappers Fleetdeck must never spawn (docs/terraria/README.md
...
function hostFrom(options = {}) {
  ...
  return {
    ...
    env: host.env && typeof host.env === 'object' ? host.env : process.env,
...
function parseReleases(variant, raw) {
  ...
  catch (_) { fail(`${SOURCES[variant].label} returned a response Fleetdeck could not read.`, 502, 'invalid_release_payload'); }
  ...
  const shape = variant === 'tmodloader' ? tmodloaderRelease : tshockRelease;
  return payload
    .filter((release) => release && !release.draft)
    .map(shape)
    // Drafts, prereleases and releases without a usable asset never reach the
    // list: an operator cannot install what Fleetdeck will not offer, and a
    ...
    if (cached) {
      return {
        ...
        error: isThrottled(error)
          ? `${source.label} is rate-limiting Fleetdeck right now. Showing the list from the last successful check.`
          : `${source.label} could not be reached. Showing the list from the last successful check.`,
      };
    }
    if (isThrottled(error)) {
      return { entries: [], retrievedAt: null, stale: true, error: `${source.label} is rate-limiting Fleetdeck right now, and Fleetdeck has no earlier list to fall back on. Try again in a few minutes.` };
    }
 ...
 * build whose compatibility Fleetdeck reports as unknown. Only a missing asset
...
function dotnetOptions(host, extra = {}) {
  return {
    ...
    ...(host.dotnetRoots ? { roots: host.dotnetRoots } : {}),
    ...extra,
  };
}
...
async function resolveDownload(variant, versionId, options = {}) {
  ...
  if (wanted && !entry) {
    fail(`That ${variant} version is not in the list Fleetdeck resolved from ${list.source}. Refresh the version list and choose again.`, 400, 'unknown_version');
  }
  if (!entry) {
...
function validateZip(archive, limits = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // in Fleetdeck goes through, would never see it.
 ...
 * the host's binary name is what selects the platform folder - Fleetdeck never
 ...
 * Fleetdeck resolves the same two pieces itself: the managed entry point in
...
function resolveTmodloaderRuntime(root, host) {
  ...
  if (!entry) fail('The tModLoader package did not contain tModLoader.dll, so Fleetdeck cannot tell how to start it.', 422, 'entrypoint_missing');
...
/*
 * The argv Fleetdeck will spawn, for one extracted tree.
 *
...
function buildLaunchPlan(variant, root, configFile, host) {
  ...
  if (variant === 'tmodloader') {
    ...
    if (!names || !names.length) fail(`Fleetdeck cannot install ${variant} on ${host.platform}.`, 409, 'unsupported_platform');
    const executable = findFile(root, [...names]);
    if (!executable) {
      fail(variant === 'tshock'
        ? 'The TShock package did not contain a TShock.Server binary, so Fleetdeck cannot tell how to start it.'
  ...
  if (WRAPPER_EXTENSIONS.has(path.extname(plan.executable).toLowerCase())) {
    fail('Fleetdeck refuses to start a Terraria server through a launcher script.', 500, 'wrapper_refused');
  }
  return plan;
}
...
function serverConfigText(settings, eol = os.EOL) {
  const lines = [
    '# Written by Fleetdeck when this server was created.',
...
async function discoverUpdate(desc, options = {}) {
  ...
  if (!current || !current.variant) {
    notes.push('Fleetdeck does not know which build this server is running, so it cannot tell whether an update exists. Reinstalling through the wizard records the version.');
  }
  if (list.stale) notes.push(list.error || 'The version list is from Fleetdeck\'s last successful check.');
