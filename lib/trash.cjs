 ...
 *      documented Fleetdeck quarantine on the same filesystem with retention
 ...
 *
 * Layout. Every entry has its manifest under the Fleetdeck data directory so a
 ...
 * from is still recoverable *by the operator*, but Fleetdeck must be honest
...
function detectOsTrash({ platform = process.platform, probe } = {}) {
  const run = probe || ((file, args) => {
    try { execFileSync(file, args, { stdio: 'ignore', timeout: 4000, windowsHide: true }); return true; } catch (_) { return false; }
  });
  if (platform === 'linux') {
...
function moveToTrash({
  ...
  // that legitimately lives inside a Fleetdeck-owned directory.
  scope = 'root',
} = {}) {
  ...
  if (scope === 'root') {
    // The same guard adoption uses: a drive root, a home root, Fleetdeck's own
  ...
  if (useOsTrash) {
    const os = detectImpl();
    if (!os.available) throw fail('This host has no usable OS trash. Use the Fleetdeck quarantine instead.', 409, 'os_trash_unavailable');
...
function cleanupEntry(manifest) {
  ...
  if (manifest.location === 'quarantine' && manifest.payloadPath) {
    ...
    try { if (!fs.readdirSync(quarantine).length) fs.rmdirSync(quarantine); } catch (_) { /* other entries remain */ }
  }
}
...
function sweep({ now = Date.now() } = {}) {
  ...
  for (const entry of list()) {
    ...
    try { purge(entry.id); purged.push(entry.id); } catch (_) { /* reported on the next sweep */ }
  }
  return purged;
}
