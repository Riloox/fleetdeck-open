 ...
 *      internet access."
 *   - "Fleetdeck may test local listeners and optionally test a user-requested
 ...
 *
 * Every item this module emits is tagged with how Fleetdeck knows it:
 *
 *   observed    - Fleetdeck measured it on this host, right now;
 ...
 *   instruction - something the operator has to do; Fleetdeck cannot verify it.
 *
 ...
 * what Fleetdeck launches; the settings file is what the game reports to
...
function configuredPorts(server) {
  ...
  } catch (_) { /* an unreadable settings file is reported as "not readable", not as zero */ }
 ...
 * the operator their configuration is wrong, and never claims Fleetdeck can
 * change any of it.
 */
function checklist({ ports, online }) {
  const items = [
    ...
    },
    {
      ...
      message: 'Allow inbound UDP on the game port through this machine\'s firewall. Fleetdeck cannot verify host firewall rules.',
    },
    {
      id: 'port_forward',
      state: 'info',
      evidence: EVIDENCE.INSTRUCTION,
      message: 'For internet play, forward the game port (UDP) on the router to this machine. Fleetdeck documents this; it never changes router configuration.',
    },
    {
      id: 'public_ip',
  ...
  if (ports.mismatch.length) {
    items.push({
      ...
      state: 'attention',
      evidence: EVIDENCE.OBSERVED,
      message: 'The ports Fleetdeck launches with and the ports in the settings file disagree. Players will use the settings value.',
    });
  }
 ...
 * prove.
 */
async function testEndpoint({ host, port, timeoutMs = PUBLIC_PROBE_TIMEOUT_MS, probeTcpImpl = probeTcp } = {}) {
  ...
  const result = await probeTcpImpl(number, { host: target, timeoutMs });
  return {
    ok: true,
