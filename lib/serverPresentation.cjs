 ...
 *
 * Spec contract:
 *   - "Allow an optional Fleetdeck-local profile icon, banner, and accent
 ...
 *
 * Assets live under the Fleetdeck data directory, never inside a server folder:
...
function readMeta(serverId) {
  try { return JSON.parse(fs.readFileSync(metaPath(serverId), 'utf8')); } catch (_) { return {}; }
}
...
/* Reset to Fleetdeck defaults: no icon, no banner, no accent. */
