...
function findPackage(server, packageId) {
  ...
  if (!found) fail('That mod is not managed by Fleetdeck.', 404, 'package_not_found');
  ...
  return new Promise((resolve, reject) => {
    yauzl.open(file, { lazyEntries: true, validateEntrySizes: true, decodeStrings: true }, (error, zip) => {
      ...
      zip.on('entry', (entry) => {
        ...
        catch (_) { return bail(new ModError('An archive entry escapes the staging root.', 422, 'path_traversal')); }
...
function classify(files) {
  ...
  if (blocked.length) {
    fail(`The archive contains executable content (${blocked[0].inside}). Fleetdeck never runs installers from an archive.`, 422, 'executable_content');
  }
 ...
 * Fleetdeck action until the operator adopts them.
 */
...
async function previewImport({ server, manager, actorId, archivePath, fileName, requestedName, provider, sourceItemId, clientRequired, license, announceSeconds = 0, host = platform.hostPlatform(), detectRun }) {
  ...
  if (occupied && !existing) {
    fail(`"${slug}" already exists in ${kind.root} and is not managed by Fleetdeck. Adopt it first, or choose another name.`, 409, 'conflict_unmanaged');
  }
