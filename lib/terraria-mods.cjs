...
function readSources(desc) {
  ...
  return value?.version === 1 && value.mods && typeof value.mods === 'object' ? value : { version: 1, mods: {} };
}
  ...
  for (const entry of files) {
    ...
    if (entry.name === 'Info') {
      ...
      if (entry.length !== entry.compressedLength) {
        ...
        catch (_) { fail('The .tmod Info entry could not be decompressed.', 422, 'tmod_info_invalid'); }
      }
...
function readEnabled(mods) {
  ...
  catch (_) { fail('Mods/enabled.json is not valid JSON.', 409, 'enabled_invalid'); }
...
function decodeHtml(value) {
  ...
  return String(value || '').replace(/&(amp|quot|#39|lt|gt);/g, (_, entity) => entities[entity]);
}
...
async function resolveWorkshop(value, { fetchImpl = global.fetch, force = false, now = Date.now() } = {}) {
  ...
  try {
    ...
    const response = await fetchImpl(WORKSHOP_ENDPOINT, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'user-agent': 'Fleetdeck/1.0' },
      body: body.toString(),
...
function steamLibraryRoots(steamcmd, { platform = process.platform, env = process.env, home = os.homedir() } = {}) {
  ...
  for (const root of [...roots]) {
    try {
      ...
      // Entries may share a line. Scan characters instead of using nested
      // regex quantifiers, avoiding pathological backtracking on VDF text.
      for (const line of vdf.split(/\r?\n/)) {
        let from = 0;
        while ((from = line.indexOf('"path"', from)) !== -1) {
          let cursor = from + 6;
          while (/\s/.test(line[cursor] || '')) cursor += 1;
          if (line[cursor] !== '"') { from += 6; continue; }
          cursor += 1;
          let value = '';
          let escaped = false;
...
async function previewImport({ desc, actorId, manager, uploadPath, originalName, source = null }) {
  ...
  const uploadRoot = path.resolve(path.join(os.tmpdir(), 'fleetdeck-terraria-uploads'));
  const resolvedUpload = path.resolve(String(uploadPath || ''));
  if (path.dirname(resolvedUpload) !== uploadRoot || !/^[0-9a-f]{32}$/i.test(path.basename(resolvedUpload))) {
    fail('The uploaded file is not in the expected staging area.', 400, 'upload_path_invalid');
  }
  const staging = fs.mkdtempSync(path.join(os.tmpdir(), 'fleetdeck-terraria-import-'));
  let files;
  try {
    if (path.extname(originalName || uploadPath).toLowerCase() === '.zip') files = await scanZip(resolvedUpload, staging);
    else {
      const target = path.join(staging, path.basename(originalName || 'upload.tmod'));
      fs.copyFileSync(resolvedUpload, target);
      files = [target];
    }
...
function importPack(desc, input) {
  ...
  catch (_) { fail('The modpack JSON is invalid.', 422, 'pack_invalid'); }
